...
- **Kill-switches (Stage 5, Phase 28):** each AI feature can be turned off without a deploy. `INSIGHT_ENABLED` / `ASCENDAI_ENABLED` are global env flags (default on); `organizations.ascendai_enabled` is a per-org toggle an owner controls from the Team panel (`PATCH /api/organizations/:id`). A disabled feature returns the same clean `status: 'unavailable'` shape a provider outage produces, so the dashboard degrades — it never errors. `GET /api/ascendai/usage` surfaces today's turn count and token totals per org. Gemini's paid AI Studio tier stays commercial-licensed with inputs not used for training; Vertex AI remains the future option for region pinning or a Google DPA.
